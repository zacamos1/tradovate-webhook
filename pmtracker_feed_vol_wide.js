...
const CONTRACTS_N = 2;
const WING_MULT   = 1.75;      // wings at 1.25x the expected 1-day move
const VIX_GATE    = 30;
const ACCT        = 5000;
...
function modelFlyVol(open, volPct, symbol){
  const ivd = volPct/100/Math.sqrt(252);
  const K = Math.round(open);
  // low-VIX wing floor: crushed IV shrinks wings but jump risk doesn't shrink
  // with it. Floor at 1.5% of spot (2.0% for IWM). Dormant above VIX ~13.6.
  const FLOOR_PCT = (symbol === 'IWM') ? 0.020 : 0.015;
...
(async () => {
  ...
  if(rows.some(r => r.date===today && r.variant==='vol_wide')){ console.log(`vol_wide variant already logged for ${today}`); process.exit(0); }
  ...
  console.log(`VOL-SCALED-WIDE variant  ${today}   (WING_MULT=${WING_MULT})`);
  ...
  for(const r of base){
    ...
    const fly = modelFlyVol(r.spot, volPct, r.symbol);
    const risk = fly.model_maxloss * CONTRACTS_N;
    const rec = { date:today, symbol:r.symbol, note:r.note, variant:'vol_wide',
