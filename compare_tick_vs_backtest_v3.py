...
# entry_ts, exit_ts (entry_ts + bars_held*5min), direction, entry, exit
trades = [
...
for entry_ts_str, bars_min, direction, entry, exit_px in trades:
    ...
    try:
        # pull a slightly wider window so we have candidates near both timestamps
        ticks = ib.reqHistoricalTicks(
            contract, startDateTime=entry_ts.strftime('%Y%m%d %H:%M:%S'),
            endDateTime='', numberOfTicks=2000, whatToShow='TRADES', useRth=False
        )
        if not ticks:
        ...
        print(f"  Closest real tick to ENTRY time ({entry_time_gap:.0f}s away): price={entry_tick.price}, backtest said {entry} -> diff {abs(entry_tick.price-entry):.2f}pts")
        print(f"  Closest real tick to EXIT time  ({exit_time_gap:.0f}s away): price={exit_tick.price}, backtest said {exit_px} -> diff {abs(exit_tick.price-exit_px):.2f}pts")
